...
## fit spherical harmonic regression for comparison 
odfs_shls = sf_to_sh(Ynorm[mask,:], Sphere(xyz=bvecs_bix),  sh_order=sh_order, basis_type="descoteaux07",  smooth=1e-3) @ T_n 

FODFCoefs_shls = odf_sh_to_sharp(odfs_shls, sphere,
								basis="descoteaux07", 
								ratio=0.3, ##should estimate this from the data or make it configurable
								sh_order=sh_order, 
								lambda_=1.0, tau=0.1, r2_term=False)
FODFCoefs_shls_tourn = sf_to_sh(FODFCoefs_shls @ B.T, sphere, sh_order=8, basis_type="tournier07") ##change coordiante system for visualizing in mrtrix
FODFCoefs_shls_tourn = FODFCoefs_shls_tourn/np.linalg.norm(FODFCoefs_shls_tourn, axis=-1)[...,None]
FODFtensor_shls = np.zeros((nx, ny, nz, K))
FODFtensor_shls[mask, :] = FODFCoefs_shls_tourn
fodf_img = nib.Nifti1Image(FODFtensor_shls, img.affine)
nib.save(fodf_img,  os.path.join(out_folder, "fodfs_shls_tournier07.nii.gz"))

ODFtensor_shls = np.zeros((nx, ny, nz, K))
ODFtensor_shls[mask, :] = odfs_shls
odf_img = nib.Nifti1Image(ODFtensor_shls, img.affine)
nib.save(odf_img,  os.path.join(out_folder, "odfs_shls.nii.gz"))
...
## compute point-wise predictions (note: this can be performed on any arbitrary set of domain points, .e.g. 'super-resolution', for simplicity, predictions are formed on observed data grid)
coords = dataloader.dataset.X.to(device)
model_output = field_model(coords)
Post_mean_coefs = model_output["model_out"]

## save point-wise posterior moments
torch.save(Post_mean_coefs, os.path.join(out_folder, "pointwise_mean_estimates.pt"))
...
## save field model
torch.save(field_model.state_dict(), os.path.join(out_folder, "nodf_model.pth"))
